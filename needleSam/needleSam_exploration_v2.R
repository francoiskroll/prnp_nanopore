...
ied <- seqmeta %>%
  select('sample', 'genebody_coverage', 'promoter_coverage') %>%
  left_join(ied, ., by='sample')
...
cohortDraws <- function(nsim) {
  ...
  for (i in 1:nsim) {
    ...
    cat ('\t \t \t \t Simulation', i, 'out of', nsim, '\n')
    cohdra <- sample(iedo$prion_disease, nrow(iedSomo)) # cohorts drawn
...
# Number of calls vs MRC intersect/slope ----------------------------------

# vs MRC_t0
# i.e. score when patient first started getting followed
callsvsMRCt0 <- ggplot(cabs, aes(x=MRC_t0, y=nSom, colour=prion_disease, alpha=gDNA_tissue)) +
  ...
  ylab('somatic mutation calls') + xlab('MRC scale at first assessment')
callsvsMRCt0

if (exportOrNo) {
  ggsave(filename=here('needleSam', 'callsvsMRCt0.pdf'), callsvsMRCt0, width=120, height=100, units='mm')
}


# with percentages
callsvsMRCt0Per <- ggplot(cabs, aes(x=MRC_t0, y=somPer, colour=prion_disease, alpha=gDNA_tissue)) +
  geom_point() +
  scale_colour_manual(values=cohcols) +
  scale_alpha_discrete(range=c(1.0, 0.5)) +
  theme_minimal() +
  ...
  coord_cartesian(xlim=c(20, 0)) +
  ylab('somatic mutation calls (% of reads)') + xlab('MRC scale at first assessment')
callsvsMRCt0Per

if (exportOrNo) {
  ggsave(filename=here('needleSam', 'callsvsMRCt0Percentage.pdf'), callsvsMRCt0, width=120, height=100, units='mm')
}


# vs MRC_intersect
  # just having a look, but I do not think there is a clear interpretation for MRC intersect
  # also it is very close to MRC_t0
callsvsMRCint <- ggplot(cabs, aes(x=MRC_intersect, y=nSom, colour=prion_disease, alpha=gDNA_tissue)) +
  geom_point() +
  scale_colour_manual(values=cohcols) +
  scale_alpha_discrete(range=c(1.0, 0.5)) +
  theme_minimal() +
...
if (exportOrNo) {
  ggsave(filename=here('needleSam', 'callsvsMRCint.pdf'), callsvsMRCint, width=120, height=100, units='mm')
}

  
# with percentages
callsvsMRCintPer <- ggplot(cabs, aes(x=MRC_intersect, y=somPer, colour=prion_disease, alpha=gDNA_tissue)) +
  geom_point() +
  scale_colour_manual(values=cohcols) +
  scale_alpha_discrete(range=c(1.0, 0.5)) +
  theme_minimal() +
...
if (exportOrNo) {
  ggsave(filename=here('needleSam', 'callsvsMRCintPercentage.pdf'), callsvsMRCint, width=120, height=100, units='mm')
}

# vs MRC_slope
  # goal is to quantify speed of decline
callsvsMRCslope <- ggplot(cabs, aes(x=MRC_slope, y=nSom, colour=prion_disease, alpha=gDNA_tissue)) +
  geom_point() +
  scale_colour_manual(values=cohcols) +
  scale_alpha_discrete(range=c(1.0, 0.5)) +
  theme_minimal() +
  ...
  ylab('somatic mutation calls') + xlab('MRC scale slope')
callsvsMRCslope

if (exportOrNo) {
  ggsave(filename=here('needleSam', 'callsvsMRCslope.pdf'), callsvsMRCslope, width=75, height=75, units='mm')
}


# with percentages
callsvsMRCslopePer <- ggplot(cabs, aes(x=MRC_slope, y=somPer, colour=prion_disease, alpha=gDNA_tissue)) +
  geom_point() +
  scale_colour_manual(values=cohcols) +
  scale_alpha_discrete(range=c(1.0, 0.5)) +
  theme_minimal() +
  ...
  coord_cartesian(xlim=c(0, -0.7)) +
  ylab('somatic mutation calls (% of reads)') + xlab('MRC scale slope')
callsvsMRCslopePer
cor(cabs$MRC_slope, cabs$somPer, use='pairwise.complete.obs')

if (exportOrNo) {
  ggsave(filename=here('needleSam', 'callsvsMRCslopePercentage.pdf'), callsvsMRCslopePer, width=75, height=75, units='mm')
}



# Number of calls vs age --------------------------------------------------

# number of somatic mutation calls vs age of patient when sample taken
  # blood samples: when blood drawn
  # brain samples: age at death
callsvsage <- ggplot(cabs, aes(x=age_atsample, y=nSom, colour=prion_disease, alpha=gDNA_tissue)) +
  geom_point() +
  scale_colour_manual(values=cohcols) +
  scale_alpha_discrete(range=c(1.0, 0.5)) +
  theme_minimal() +
  ...
  xlab('age of patient') + ylab('somatic mutation calls')
callsvsage
cor(cabs$nSom, cabs$age_atsample, method='pearson', use='pairwise.complete.obs')
cor(cabs$nSom, cabs$age_atsample, method='spearman', use='pairwise.complete.obs')

if (exportOrNo) {
  ggsave(filename=here('needleSam', 'callsvsage.pdf'), callsvsage, width=120, height=100, units='mm')
}

# with percentages
callsvsagePer <- ggplot(cabs, aes(x=age_atsample, y=somPer, colour=prion_disease, alpha=gDNA_tissue)) +
  geom_point() +
  scale_colour_manual(values=cohcols) +
  scale_alpha_discrete(range=c(1.0, 0.5)) +
  theme_minimal() +
  theme(
    ...
    axis.title.y=element_text(size=9, margin = margin(t=0, r=2, b=0, l=0)),
    axis.text.y=element_text(size=7, margin = margin(t=0, r=0, b=0, l=0))) +
  xlab('age of individual') + ylab('somatic mutation calls (% of reads)')
callsvsagePer

cor(cabs$somPer, cabs$age_atsample, method='pearson', use='pairwise.complete.obs')
...
if (exportOrNo) {
  ggsave(filename=here('needleSam', 'callsvsagePercentage.pdf'), callsvsagePer, width=75, height=75, units='mm')
}
...
# Does age affect number of somatic calls irrespective of OPR? ------------

# first; is there a correlation OPR vs age in our dataset
  # we are assuming here age at sample is a proxy for age at onset
  # from samples for which we have both age at onset + age at sample + age at death, can see that it correlates well
  # however there are 4 individuals for whom this is evidently incorrect
  # 2 controls + one 1 OPRI + one 6 OPRI
  # (I imagine the OPRI ones are patients screened then followed over time but were not sick at time of recording)
  # can tell these samples as they have an MRC slope of 0 (as not sick)
  # it is possible that there is a couple of other samples like this
    # especially when the individual is dead but we do not have information about disease progression
    # it may be that they were being followed because they had an OPR mutation but died from something else entirely
  # however, I can still see for some of those that sample was taken on the year of death or the year before
  # which strongly suggests this was because they were sick

# exclude individuals which are not sick (so age at sample does not represent onset)
onidv <- as.numeric(unlist(cabs[which(cabs$MRC_slope==0), 'individual'])) # onidv for onset individuals
cabson <- subset(cabs, ! individual %in% onidv)

# fit, lm(y ~ x)
agefit <- lm(cabson$age_atsample ~ cabson$sanOPRdiff)
agefit
# age onset = 70 - 1.46 * sanOPRdiff
# i.e. if reference OPR: age at onset is 70
# less repeats increase age at onset (i.e. delay onset)
# more repeats decrease age at onset (i.e. advance onset)
# for each extra repeat, onset is 1.5 years earlier
# (need to highlight this is a poor fit though; but replicates literature, see below)
summary(agefit)

intercept <- agefit[[1]][1]
slope <- agefit[[1]][2]

# X start
# is shortest OPR
xstart <- min(cabson$sanOPRdiff)

# X stop
# is longest OPR
xstop <- max(cabson$sanOPRdiff)
...
sanOPRdiffvsage <- ggplot(cabson, aes(x=sanOPRdiff, y=age_atsample, colour=prion_disease)) +
  geom_segment(aes(x=xstart, xend=xstop,
                   y=ystart, yend=ystop),
               linetype=2, colour='#ebebeb', size=1.0) +
  geom_quasirandom(width=0.3) +
  ...
  xlab('OPR change vs reference') + ylab('age of patient')
sanOPRdiffvsage

if (exportOrNo) {
  ggsave(filename=here('needleSam', 'sanOPRdiffvsage.pdf'), sanOPRdiffvsage, width=120, height=100, units='mm')
}
  
cor(x=cabson$sanOPRdiff, y=cabson$age_atsample, method='pearson')
cor.test(x=cabson$sanOPRdiff, y=cabson$age_atsample, method='pearson')

cor(x=cabson$sanOPRdiff, y=cabson$age_atsample, method='spearman')
cor.test(x=cabson$sanOPRdiff, y=cabson$age_atsample, method='spearman')
# there is a NON-significant negative correlation
  # negative is what we would expect; more repeats give lower age at onset

# this looks fairly similar as Figure 1: https://jnnp.bmj.com/content/75/8/1166
# which is reassuring

# get the residuals from fit above
  # = portion of onset age not explained by OPR
  # e.g. for individual 3, onset is higher (delayed) vs what we would expect from their OPR (positive residual)
cabson$sanOPRdiff_vs_age_residuals <- resid(agefit)

# add the column to cabs by matching samples
cabs <- cabson %>%
  select(sample, sanOPRdiff_vs_age_residuals) %>%
  left_join(x=cabs, y=., by='sample')

# similarly, we have fit OPR vs % somatic calls from before
# get the residuals
  # = portion of % somatic that is not explained by OPR
  # e.g. for this 8 OPRI individual, we would have expected many calls but it does not have a lot (negative residual)
cabs$sanOPRdiff_vs_somPer_residuals <- resid(somfit) # we can directly add those to cabs


# Approach 1
# Is there a correlation somatic % residuals vs age?
  # I think this is assuming there is no relationship age vs OPR which is probably untrue
# positive correlation:
  # less mutations than expected from OPR match younger age &&
  # more mutations ... match older age
    # >>> so more mutations as age increases, irrespective of OPR
  # and vice-versa; negative correlation >>> less mutations as age increases, irrespective of OPR
ageVsMutationResidual <- ggplot(cabs, aes(x=age_atsample, y=sanOPRdiff_vs_somPer_residuals, colour=prion_disease)) +
  geom_point() +
  scale_colour_manual(values=cohcols) +
  theme_minimal() +
  theme(
  ...
  xlab('age of patient') + ylab('% somatic calls unexplained by OPR')
ageVsMutationResidual

cor(x=cabs$age_atsample, y=cabs$sanOPRdiff_vs_somPer_residuals, method='pearson')
cor.test(x=cabs$age_atsample, y=cabs$sanOPRdiff_vs_somPer_residuals, method='pearson')

cor(x=cabs$age_atsample, y=cabs$sanOPRdiff_vs_somPer_residuals, method='spearman')
cor.test(x=cabs$age_atsample, y=cabs$sanOPRdiff_vs_somPer_residuals, method='spearman')
# NON-significant negative correlation (or none with Spearman)
# would (vaguely) suggest less mutations as age increases

# Approach 2
# residuals vs residuals
# i.e. x = portion of age unexplained by OPR vs y = portion of % somatic unexplained by OPR
  # positive correlation:
    # younger onset than expected by OPR match less mutations than expected by OPR && 
    # vice-versa; older onset ... more mutations
    # >>> interpretation could be mutations are protective; i.e. they delay onset
  # negative correlation:
    # older onset ... more mutations &&
    # younger onset ... less mutations
    # # >>> interpretation could be mutations are detrimental; i.e. they accelerate onset

ageResidualsVsMutationResiduals <- ggplot(cabs, aes(x=sanOPRdiff_vs_age_residuals, y=sanOPRdiff_vs_somPer_residuals, colour=prion_disease)) +
  geom_point() +
  scale_colour_manual(values=cohcols) +
  theme_minimal() +
  theme(
  ...
  xlab('difference in age unexplained by OPR') + ylab('% mutation unexplained by OPR')
ageResidualsVsMutationResiduals

cor(x=cabs$sanOPRdiff_vs_age_residuals, y=cabs$sanOPRdiff_vs_somPer_residuals, use='complete.obs', method='spearman')
cor.test(x=cabs$sanOPRdiff_vs_age_residuals, y=cabs$sanOPRdiff_vs_somPer_residuals, use='complete.obs', method='spearman')

cor(x=cabs$sanOPRdiff_vs_age_residuals, y=cabs$sanOPRdiff_vs_somPer_residuals, use='complete.obs', method='pearson')
cor.test(x=cabs$sanOPRdiff_vs_age_residuals, y=cabs$sanOPRdiff_vs_somPer_residuals, use='complete.obs', method='pearson')
# not convincing at all -- but if anything; negative correlation
# which would suggest somatic mutations accelerate onset irrespective of OPR

# from labels + 2 approaches here, I think nothing particularly convincing
